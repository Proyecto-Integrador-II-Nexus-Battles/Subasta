...
export class CompraController {
  static async enviarCompra(req, res) {
    try {
      ...
      const ID_CARTA = await pool.query(
        "SELECT ID_CARTA FROM CARTA_SUBASTA WHERE ID = ?",
        [ID_SUBASTA]
      );
      let datos;
      axios
        ...
        .then(() => {
          ...
          axios
            .get(`/subasta/getSubasta/${ID_SUBASTA}`)
            .then((response) => {
              datos = response.data;
              axios
                .post("/subasta/buzon/add", {
                ...
                .then(() => {
                  console.log(
                    "Creditos agregados al buzon correctamente del subastador"
                  );
                })
                .catch((error) => {
                  console.error(
                    "Error al agregar los creditos al buzon del subastador:",
                    error
                  );
                });
              if (datos[0].CARTAS_MAX !== null) {
                ...
                datos[0].CARTAS_MAX.forEach((carta) => {
                  axios
                    ...
                    .then(() => {
                      console.log(
                        "Carta agregada al buzon correctamente Maximas"
                      );
                      cartica.push({
                        CARTA_ID: carta.ID,
                      ...
                      axios
                        .delete(`${HOST}:${PORT}/inventario/delete/cards`, {
                          data: { CARTAS: cartica },
                          headers: {
                            Authorization: req.headers.authorization,
                          },
                        ...
                        .then((response) => {
                          console.log(
                            "Carta eliminada correctamente, respuesta:",
                            response
                          );
                        })
                        .catch((error) => {
              ...
              axios
                ...
                .then(async (response) => {
                  if (response.status === 200) {
                    await CheckTime.sendMail(ID_CARTA[0].ID_CARTA, IdUsuario);
                    console.log("Creditos eliminados correctamente");
                    axios
                      .get(`/subasta/deleteSubasta/${ID_SUBASTA}`)
                      .then(() => {
