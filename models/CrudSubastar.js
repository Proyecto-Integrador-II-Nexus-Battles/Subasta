...
export class crudSubastar {
    ...
    static async INSERT_CARD_SUBASTA(
        ...
        CANTIDAD_CARD_MIN){
            ...
            try{
                const resultInsertCardMax = await this.INSERT_CARD_MAX(ID_CARD_MAX, CANTIDAD_CARD_MAX);

                const resultInsertCardMin = await this.INSERT_CARD_MIN(ID_CARD_MIN, CANTIDAD_CARD_MIN);
              ...
              `;
                ...
                const result = await pool.query(insertQuery, [Number(ID_USUARIO), ID_CARD, Number(TIEMPO), Number(CREDITOS_MIN), Number(CREDITOS_MAX), Number(resultInsertCardMax), 
                ...
                const delete_creditos = await axios.post(
                    `${HOST_G}:${PORT_G}/inventario/delete-creditos`,
                    { ID_USUARIO,  CANTIDAD},
                );
